...
try {
    ...
    if ($listing['listerID'] != $userID) {
        sendJsonResponse(['error' => 'Unauthorized to edit this listing'], 403);
    }
    ...
    if (!$updateStmt->execute()) {
        throw new Exception('Failed to update listing: ' . $db->lastErrorMsg());
    }
    
    sendJsonResponse(['message' => 'Listing updated successfully']);
